import { NativeBaseProvider, AlertDialog, Collapse, Text, ZStack, Alert, Center, Box, Button, HStack, Image, IconButton, Tooltip, FormControl, Input, VStack, TextArea} from "native-base";
import '..//App.css';
...
const Contact = () => {
    ...
    return (
        <NativeBaseProvider>
          ...
          {/* Contact */}
          <Center paddingTop={'3rem'} h={'full'} justifyContent={'center'} justifyItems={'center'}>
            <Box flexDirection={'row'} alignItems={'center'} alignContent={'center'} justifyItems={'center'}>
              <RiCustomerService2Fill size={'30px'} color={"#8E9D7D"} />
              <Text justifyContent={'center'} fontSize={'30px'} fontFamily={'cwTeXKai'} fontWeight={600} color={'#8E9D7D'}> 
                  &ensp;與我們聯絡 
              </Text>
            </Box>
                ...
                <VStack space={1} flexShrink={1} w="100%">
                  <HStack flexShrink={1} space={2} alignItems="center" justifyContent="space-between">
                    <HStack flexShrink={1} space={2} alignItems="center">
                      <Alert.Icon />
                      <Text fontSize="xl" fontWeight="medium" _dark={{
                      color: "coolGray.800"
                    }} fontFamily={'cwTeXKai'}>
                        寄送失敗
                      </Text>
            ...
            <VStack w={'60%'} justifyContent={'center'}>
              <FormControl isRequired isInvalid={'name' in errors}>
                ...
                {'name' in errors ? <FormControl.ErrorMessage leftIcon={<BiErrorCircle size="18px" color="#e60000"/>}>Error</FormControl.ErrorMessage> : <FormControl.HelperText>
                  ...
                  <FormControl.Label _text={{
                    bold: true
                  }}>電子信箱 Email</FormControl.Label>
                    <Input placeholder="john@gmail.com" onChangeText={value => setData({ ...formData,
                    email: value
                  })} />
                  <FormControl.HelperText _text={{
                  fontSize: 'xs'
                  }}>
                      "有效電子信箱需包含'@'和'.'  (An valid email address at least contain '@' and '.')"
                  </FormControl.HelperText>
                ...
                <FormControl.Label _text={{
                bold: true
              }}>您想與我們說的話 Content</FormControl.Label>
              ...
              <Button onPress={() => setIsOpen(!isOpen)} mt="5" colorScheme="cyan">
                ...
                <AlertDialog leastDestructiveRef={cancelRef} isOpen={isOpen} onClose={onClose}>
                  <AlertDialog.Content>
                    ...
                    <AlertDialog.Header _text={{fontFamily: 'cwTeXKai'}}>確定送出嗎？ Are you sure?</AlertDialog.Header>
                    <AlertDialog.Body _text={{fontFamily: 'cwTeXKai'}}>
                      請在送出前確認您的資訊是否正確。送出後將無法修改。
                      (Make sure you check your information before submitting. After submitting, you won't be able to edit it.)
                    </AlertDialog.Body>
                    <AlertDialog.Footer>
                      <Button.Group space={2}>
                        <Button colorScheme="danger" onPress={onClose} ref={cancelRef}>
                          Cancel
                        </Button>
